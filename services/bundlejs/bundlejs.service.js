...
const keywords = ['node', 'bundlejs', 'esbuild', 'bundlephobia']

export default class Bundlejs extends BaseJsonService {
  ...
  static render({ format, size }) {
    const label = prettyBytes(size)
    return {
      label,
      message: format === 'min' ? 'minified size' : 'minzipped size',
    }
  }
